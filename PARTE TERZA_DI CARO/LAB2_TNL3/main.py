...
#Analisi della similarità semantica tramite simlex
    ...
    risultati_simlex=simlex.compute_simlex_for_concept(definizioni_dict)
    for categoria, dati in risultati_simlex.items():
        media = dati["media_similarità"]
        matrice = dati["matrice_similarità"]
        tipo_specificità=dati["specificità"]
        tipo_concretezza=dati["concretezza"]
        print(f"{categoria}: media = {media}")
        u.plot_similarity_matrix(matrice, categoria, tipo_concretezza, tipo_specificità, similarity_type="lexical")
...
#Analisi della similarità tramite simsem
    ...
    for categoria, dati in risultati_simlex.items():
        ...
        tipo_specificità=dati["specificità"]
        tipo_concretezza=dati["concretezza"]
        print(f"{categoria}: similarità media = {media}")
        u.plot_similarity_matrix(matrice, categoria, tipo_concretezza, tipo_specificità, similarity_type="semantic")
